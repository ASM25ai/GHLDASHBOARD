...
async function getAccessToken() {
  ...
  const clientSecret = process.env.HUBSTAFF_CLIENT_SECRET;
  if (!clientId)     throw new Error('HUBSTAFF_CLIENT_ID env var is not set.');
  if (!clientSecret) throw new Error('HUBSTAFF_CLIENT_SECRET env var is not set.');

  // Hubstaff's token endpoint follows the OAuth 2.0 standard and requires
  // application/x-www-form-urlencoded, NOT JSON (even though the rest of
  // the Hubstaff API uses JSON). Using JSON here causes a 400 error that
  // misleadingly says "client_id required" even when client_id is present.
