...
async function ghlFetch(path, options = {}, retries = 3) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    const timeout = setTimeout(() => controller.abort(), 15000); // 15s per request max
    ...
    try {
      ...
      console.log(`GHL ${path} page=${JSON.parse(options.body || '{}').page || 1} → ${res.status} (${elapsed}ms)`);
      ...
      throw new Error(`GHL API error ${res.status} on ${path}: ${body.slice(0, 200)}`);
    } catch (err) {
      clearTimeout(timeout);
      if (err.name === 'AbortError') {
        const elapsed = Date.now() - start;
        console.warn(`GHL ${path} timed out after ${elapsed}ms (attempt ${attempt}/${retries})`);
        if (attempt < retries) {
          await new Promise((r) => setTimeout(r, attempt * 2000));
          continue;
        }
        throw new Error(`GHL API timeout on ${path} after ${retries} attempts`);
      }
      throw err;
    }
  }
...
// rangeStart and rangeEndExclusive should be Date objects — .getTime() is used
// for the GHL ms-based range filter.
async function fetchQualifiedLeadsDayByDay(locationId, fieldId, rangeStartMs, rangeEndMs) {
  const allContacts = await searchAllContactsByDateRange(locationId, fieldId, rangeStartMs, rangeEndMs);
...
async function fetchNewLeadsDayByDay(locationId, createdDateFieldId, rangeStartMs, rangeEndMs) {
  const allContacts = await searchAllContactsByDateRange(locationId, createdDateFieldId, rangeStartMs, rangeEndMs);
