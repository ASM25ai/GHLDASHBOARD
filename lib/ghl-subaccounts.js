...
// GHL returns customFields as: [{ id: "xxx", key: "lead_tier", value: "paid" }, ...]
// We match by key (the human-readable name set in GHL custom fields)

function getCustomFieldValue(contact, fieldKey) {
  const fields = contact.customFields || contact.customField || [];
  for (const f of fields) {
    // Match by key (e.g. "lead_tier") or by field_key
    if (
      (f.key && f.key.toLowerCase() === fieldKey.toLowerCase()) ||
      (f.field_key && f.field_key.toLowerCase() === fieldKey.toLowerCase())
    ) {
      return f.value || '';
    }
  }
...
async function aggregateDealerStats(config) {
  ...
  if (splitField) console.log(`  Split field: ${splitField} (paid value: "${splitPaidValue}")`);
  ...
  for (const c of allTimeContacts) {
    ...
    let isPaid = false;
    if (splitField) {
      const cfValue = getCustomFieldValue(c, splitField);
      isPaid = cfValue && cfValue.toLowerCase().trim() === splitPaidValue.toLowerCase();
    }
  ...
  return {
    ...
    allTime: {
      ...
      split: splitField ? splitAllTime : null,
    },
    thisMonth: {
      ...
      byDay: byDayMonth,
      split: splitField ? splitMonth : null,
    },
  };
}
