...
async function aggregateDealerStats(config) {
  const {
    ...
    fmOrderDates = {},           // { "FM Name": "YYYY-MM-DD" } — per-FM order start
  } = config;

  // Parse per-FM order dates into Date objects once. Keyed by lowercased FM name.
  const fmOrderStart = {};
  for (const [fmName, dateStr] of Object.entries(fmOrderDates)) {
    if (!dateStr) continue;
    const d = new Date(dateStr);
    if (!isNaN(d.getTime())) fmOrderStart[fmName.toLowerCase()] = d;
  }
  ...
  // Also fetch contacts with "refund" tag separately — some dealers remove
  // the "qualified" tag when refunding, so these contacts wouldn't be in
  // allTimeContacts. We count them as refunds but NOT as delivered.
  let refundOnlyContacts = [];
  try {
    console.log(`  Fetching contacts with "refund" tag...`);
    const allRefunds = await searchContactsByTag(locationId, apiKey, 'refund');
    // Filter out contacts already in allTimeContacts (those have both qualified + refund)
    const qualifiedIds = new Set(allTimeContacts.map((c) => c.id));
    refundOnlyContacts = allRefunds.filter((c) => !qualifiedIds.has(c.id));
    console.log(`  Refund-only contacts (no qualified tag): ${refundOnlyContacts.length}`);
    console.log(`  Refund contacts with qualified tag: ${allRefunds.length - refundOnlyContacts.length}`);
  } catch (err) {
    console.warn(`  Failed to fetch refund contacts: ${err.message}`);
  }
  ...
  // Per-FM counts since that FM's own order date (for current-order reporting).
  const byFMSinceOrder      = {};
  const refundsByFMSinceOrder = {};
  ...
  for (const c of allTimeContacts) {
    ...
    // Since-order-date count (per FM). If this FM has an order date and the
    // contact was created on/after it, count it toward the current order.
    const orderStart = fmOrderStart[(fmName || '').toLowerCase()];
    if (orderStart && created >= orderStart) {
      byFMSinceOrder[fmName] = (byFMSinceOrder[fmName] || 0) + 1;
      if (isRefund) {
        refundsByFMSinceOrder[fmName] = (refundsByFMSinceOrder[fmName] || 0) + 1;
      }
    }
    if (hasSplitConfig) {
  ...
  // Count refund-only contacts (those with refund tag but no qualified tag)
  for (const c of refundOnlyContacts) {
    const created = new Date(c.dateCreated || c.dateAdded);
    const fmName  = resolveFMFromTags(c, fmTagMap);
    ...
    // Refund-only contact toward this FM's current order, if on/after its date.
    const orderStartR = fmOrderStart[(fmName || '').toLowerCase()];
    if (orderStartR && created >= orderStartR) {
      byFMSinceOrder[fmName] = (byFMSinceOrder[fmName] || 0) + 1;
      refundsByFMSinceOrder[fmName] = (refundsByFMSinceOrder[fmName] || 0) + 1;
    }
  }

  if (refundOnlyContacts.length > 0) {
    console.log(`  Refunds updated (incl. refund-only): ${totalRefundsAllTime} → ${JSON.stringify(refundsByFMAllTime)}`);
  }
  ...
  return {
    ...
    // Per-FM counts since each FM's own order date (current-order reporting).
    sinceOrder: {
      byFM: byFMSinceOrder,
      refundsByFM: refundsByFMSinceOrder,
    },
  };
}
