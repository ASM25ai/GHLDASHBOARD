...
// Uses formulas + dropdown referencing "Current Month Data" tab
// ---------------------------------------------------------------------------
...
async function initDealerViewTab(sheets, spreadsheetId) {
  ...
  // Formulas reference "Current Month Data" tab (permanent name, always current month)
  ...
  // FM breakdown QUERY (rows 6+, up to 20 FMs — rows 7-25 are intentionally blank buffer)
  ...
  // Lead details QUERY (starts at fixed row 28 — well below the FM breakdown buffer)
  const leadsQuery = `=IFERROR(QUERY('Current Month Data'!A:J,"SELECT B,E,D,F,G WHERE C='"&B1&"' ORDER BY B LABEL B 'Date', E 'Sales Rep', D 'FM', F 'Lead Type', G 'Contact Name'",0),"Select a dealer above")`;

  // 21 blank buffer rows between FM breakdown (row 6) and lead details (row 28)
  const buffer = Array.from({ length: 21 }, () => []);

  const rows = [
    ['Select Dealer →', ''],                                          // Row 1: A1 label, B1 dropdown
    [],                                                               // Row 2
    ['Monthly Order', orderFormula, '', 'MTD Delivered', mtdFormula, '', 'Today', todayFormula, '', 'Remaining', remainFormula, '', '% Complete', pctFormula], // Row 3
    [],                                                               // Row 4
    ['── FM Breakdown ──'],                                           // Row 5
    [fmQuery],                                                        // Row 6 (expands down)
    ...buffer,                                                        // Rows 7-27
    ['── Lead Details (this month) ──'],                              // Row 28
    [leadsQuery],                                                     // Row 29 (expands down)
  ];
