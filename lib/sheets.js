...
async function readSettings(sheets, spreadsheetId) {
  ...
  try {
    const res = await sheets.spreadsheets.values.get({
      spreadsheetId,
      range: 'Settings!A2:I500',
    });
    rows = res.data.values || [];
  } catch {
    return { dealers: [], orders: {}, aliasMap: {}, fms: {}, groups: {}, refunds: {} };
  }
  ...
  const aliasMap = {};
  const fms      = {};
  const groups   = {};   // dealer → parent group name (e.g. 'South Trail Kia')
  const refunds  = {};   // dealer → number, OR 'dealer::FM Name' → number
  ...
  for (const row of rows) {
    ...
    const colF = (row[5] || '').trim();            // Group
    const colG = (row[6] || '').toString().trim();  // Refunds
    const colH = (row[7] || '').toString().trim();  // Prev Balance (FM carryover)
    const colI = (row[8] || '').trim();            // Order Date (per-FM, YYYY-MM-DD)

    if (!colA && !colB && !colC && !colD && !colE && !colF && !colG) continue;
    ...
    if (colA) {
      ...
      if (colF) groups[currentDealer] = colF;
      // Dealer-level refunds (when no FM rows)
      if (colG && !colD) refunds[currentDealer] = parseInt(colG.replace(/,/g, ''), 10) || 0;
    }

    if (colD && currentDealer) {
      const target = parseInt(colE.replace(/,/g, ''), 10) || 0;
      const fmRefunds = colG ? (parseInt(colG.replace(/,/g, ''), 10) || 0) : 0;
      // Prev Balance (col H) can be negative, so parse as a signed int.
      const prevBalance = colH ? (parseInt(colH.replace(/,/g, ''), 10) || 0) : 0;
      // Order Date (col I) — per-FM start date for counting delivered.
      const orderDate = colI || null;
      fms[currentDealer].push({ name: colD, target, prevBalance, orderDate });
      if (fmRefunds) refunds[`${currentDealer}::${colD}`] = fmRefunds;
    }
  }
...
async function initSettingsTab(sheets, spreadsheetId, seedDealers) {
  ...
  const rows = [
    ['Dealer', 'Monthly Order', 'Aliases (comma-separated — any of these in GHL count toward this dealer)', 'FM Name', 'FM Target', 'Group', 'Refunds'],
...
// ---------------------------------------------------------------------------
// Column-range color formatting for Daily Leads Breakdown
// ---------------------------------------------------------------------------

async function applyColumnColors(sheets, spreadsheetId, tabName, colorRanges) {
  // colorRanges: [{ startCol, endCol, color: {red,green,blue} }]
  // startCol/endCol are 0-based column indices (endCol exclusive)
  const meta     = await sheets.spreadsheets.get({ spreadsheetId });
  const tabSheet = (meta.data.sheets || []).find((s) => s.properties.title === tabName);
  if (!tabSheet) return;
  ...
  for (const { startCol, endCol, color, headerColor } of colorRanges) {
    // Header row (row 0) — darker shade
    if (headerColor) {
      requests.push({
        repeatCell: {
    ...
    // Data rows — lighter shade
    requests.push({
      repeatCell: {
        range: { sheetId, startRowIndex: 1, endRowIndex: rowCount, startColumnIndex: startCol, endColumnIndex: endCol },
        cell: {
          userEnteredFormat: {
  ...
  // Bold the TOTAL row (last row with data)
  // We'll bold all cells in it via a separate call after writing
...
// Bold a specific row (0-based index)
async function boldRow(sheets, spreadsheetId, tabName, rowIndex) {
  const meta     = await sheets.spreadsheets.get({ spreadsheetId });
  const tabSheet = (meta.data.sheets || []).find((s) => s.properties.title === tabName);
  if (!tabSheet) return;

  const sheetId = tabSheet.properties.sheetId;
  const colCount = (tabSheet.properties.gridProperties || {}).columnCount || 26;
...
// ---------------------------------------------------------------------------
// MTD Summary formatting — ensures Remaining column is number (not %),
// header/total rows are bold, and columns are properly formatted
// ---------------------------------------------------------------------------

async function formatMTDSummary(sheets, spreadsheetId, summaryRows) {
  const meta     = await sheets.spreadsheets.get({ spreadsheetId });
  const mtdSheet = (meta.data.sheets || []).find((s) => s.properties.title === 'MTD Summary');
  if (!mtdSheet) return;

  const sheetId  = mtdSheet.properties.sheetId;
  const rowCount = summaryRows.length;
  const requests = [];

  // 1. Reset entire sheet to plain number format (removes any leftover % format)
  requests.push({
    repeatCell: {
  ...
  // 2. Set column G (% Complete, index 6) to plain text so "9%" stays as text
  //    (it's already formatted as a string by pct())
  requests.push({
    repeatCell: {
      range: { sheetId, startRowIndex: 0, endRowIndex: rowCount, startColumnIndex: 6, endColumnIndex: 7 },
      cell: { userEnteredFormat: { numberFormat: { type: 'TEXT' } } },
      fields: 'userEnteredFormat.numberFormat',
    },
  });

  // 3. Set column A to plain text
  requests.push({
    repeatCell: {
      range: { sheetId, startRowIndex: 0, endRowIndex: rowCount, startColumnIndex: 0, endColumnIndex: 1 },
      cell: { userEnteredFormat: { numberFormat: { type: 'TEXT' } } },
      fields: 'userEnteredFormat.numberFormat',
    },
  });

  // 4. Bold section headers (═══) and TOTAL rows
  for (let i = 0; i < summaryRows.length; i++) {
    const cellA = String(summaryRows[i][0] || '');
    if (cellA.startsWith('═══') || cellA.startsWith('TOTAL') || cellA === 'GRAND TOTAL') {
      requests.push({
        repeatCell: {
