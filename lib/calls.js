...
// Fetch an entire date range of messages in ONE call.
// limit=10000 handles full month volume.
async function exportRange(locationId, channel, startDate, endDate) {
  ...
  const params = new URLSearchParams({
    locationId, channel, startDate: start, endDate: end, limit: '10000',
  });
  ...
  try {
    ...
    console.warn(`Export failed for ${channel} ${start}→${end}: ${e.message.slice(0, 100)}`);
    return [];
  }
}
...
async function fetchCallStats(reps, locationId, now, monthStart) {
  ...
  const [allCalls, allSMS] = await Promise.all([
    exportRange(locationId, 'Call', monthStart, today),
    exportRange(locationId, 'SMS',  monthStart, today),
  ]);
