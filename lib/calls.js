...
async function getMsgsFromConv(convId, sinceMs) {
  try {
    ...
    for (const m of msgs) {
      ...
      // Capture ALL message types we see (for debug, regardless of date)
      if (getMsgsFromConv._sampleTypes) {
        getMsgsFromConv._sampleTypes.add(m.messageType || m.type || 'UNKNOWN');
      }
      if (dt < sinceMs) continue;

      // Capture sample messages from today (for debug)
      if (getMsgsFromConv._sampleMsgs && getMsgsFromConv._sampleMsgs.length < 5) {
        getMsgsFromConv._sampleMsgs.push({
          type: m.messageType || m.type,
          userId: m.userId || m.user_id,
          dateAdded: m.dateAdded,
          hasCallInType: isCallType(m.messageType || m.type || ''),
        });
      }
...
async function fetchCallStats(reps, locationId, now, monthStart) {
  ...
  // Attach debug collectors to the function
  getMsgsFromConv._sampleTypes = new Set();
  getMsgsFromConv._sampleMsgs  = [];
  ...
  const sampleTypes  = Array.from(getMsgsFromConv._sampleTypes);
  const sampleMsgs   = getMsgsFromConv._sampleMsgs;
  // Also grab a raw conversation sample
  const sampleConvFields = todayConvs.length > 0 ? {
    lastMessageType: todayConvs[0].lastMessageType,
    lastMessageDate: todayConvs[0].lastMessageDate,
    dateUpdated:     todayConvs[0].dateUpdated,
    dateAdded:       todayConvs[0].dateAdded,
    messageTypes:    todayConvs[0].messageTypes,
  } : null;
  ...
  result._debug = {
    ...
    // Key debug: what message types exist in today's conversations?
    // If this is empty or missing call-type strings, that's the bug.
    msgTypesFoundToday:    sampleTypes.slice(0, 20),
    sampleMsgsFromToday:   sampleMsgs,
    sampleConvFields,
  };
