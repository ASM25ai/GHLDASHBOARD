...
// Uses /conversations/search which supports the assignedTo + startAfterDate filters.
async function getConversations(locationId, ghlUserId, sinceMs) {
  ...
  for (let page = 0; page < 10; page++) {
    ...
    // Try /conversations/search first, fall back to /conversations/ if 404
    let data;
    try {
      data = await ghlGet(`/conversations/search?${params}`);
    } catch (e1) {
      if (e1.message.includes('404')) {
...
async function fetchCallStats(reps, locationId, now, monthStart) {
  ...
  for (const [ghlName, rep] of Object.entries(reps)) {
    try {
      ...
      console.warn(`Call stats failed for ${ghlName}:`, errMsg);
      stats[ghlName] = {
        callsToday: `ERR`, calls30sToday: '-', avgDurToday: errMsg,
        callsMTD:   `ERR`, calls30sMTD:   '-', avgDurMTD:   errMsg,
      };
    }
  }
