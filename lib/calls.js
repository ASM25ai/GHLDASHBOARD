...
async function fetchCallStats(reps, locationId, now, monthStart) {
  ...
  const todayCalls = await exportMessages(locationId, 'Call', todayStr, tomorrowStr, 10);
  const todaySMS   = await exportMessages(locationId, 'SMS',  todayStr, tomorrowStr, 10);
 ...
 const mtdCalls = await exportMessages(locationId, 'Call', mtdStartStr, tomorrowStr, 10);
  const mtdSMS   = await exportMessages(locationId, 'SMS',  mtdStartStr, tomorrowStr, 10);
