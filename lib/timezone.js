...
// GHL date picker fields return YYYY-MM-DD (bare dates with no timezone).
// These should be interpreted as ET dates, not UTC — otherwise midnight UTC
// becomes the previous day in Eastern Time.
function parseDate(value) {
  if (!value) return null;

  // If it's a number (ms timestamp), convert to ET
  ...
  // Bare date: YYYY-MM-DD — treat as ET midnight (not UTC)
  if (/^\d{4}-\d{2}-\d{2}$/.test(str)) {
    const [y, m, d] = str.split('-').map(Number);
    return new Date(y, m - 1, d); // local = ET-equivalent on Vercel? No — server is UTC
    // Actually we need to just return a Date with these values for display purposes
  }

  // MM/DD/YYYY — also bare date
  if (/^\d{2}\/\d{2}\/\d{4}$/.test(str)) {
    const [m, d, y] = str.split('/').map(Number);
    return new Date(y, m - 1, d);
  }

  // Full ISO or other format — convert to ET
